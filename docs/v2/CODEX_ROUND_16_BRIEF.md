...
| at | n | MAE | hit01 | hit02 | A Step 1 변경 |
|---|---|---|---|---|---|
...
> ⚠ **메모리 baseline 표기와 실측 불일치 (§1 Q6)**: 메모리 `project_next_session_m33.md`에 "한전 LOCK baseline 30d hit01 16%, 지자체 10.9%" 적혀 있으나 위 실측은 한전 0%(n=16) / 지자체 1.15%. **다른 hit 정의(자사 1위 자기 적중 vs 사정률 ±0.01 적중)이거나 측정 식 오기** — 라운드 16 평가 시 정의 확정 권고.
...
- **A Step 2 진입 조건 (지자체×ba 2D 테이블) 정량 트리거** — 코덱스 §9.7 권고 "7일 누적 + 핵심 at 회귀 없음 + 지자체 개선 방향 확인" 중 "지자체 개선 방향"의 정량 정의 (hit01 +XX pp? floor_pass_rate +YY pp?)
...
- ⏳ **A Step 1 7일 windowed hit01 (Q2 입력)** — 5/26 `/accuracy` 재측정: **최신 스코어링이 5/22까지**(5/22 push 신규추천 아직 미매칭) → 효과 판정 데이터 미생성. **7~14일 누적 후(≥6/1) 재측정 필요**. **한전 LOCK 회귀 없음 확인**(60d hit01 ~74% via 체크8 / 30d 핵심영역 MAE 0.4173·bias +0.1331). 군부대 bias +0.2448(⚠)·hit≈0%는 구조적, A Step 1 무관.
