...
- **Phase 2a — `recommendModeA` 교체**: floorErr 분위수 + alpha 제약. **adj↔bid_rate 공간 정합** (최대 리스크). 호출부 동시 수정. **[✅ 완료·배포 2026-05-24, push d2dd667]** 4파일(modeResolver `resolveFloorErrDist` + utils `recommendModeA`/`recommendV2` Mode A 분기 + `_invertBidCToAdj` + App.jsx 적재부). m_star=clamp(p85@α=0.15 LOCK, max(0,p50), p95)=0.007448(current). 투찰금=ceil(pred_ep×pred_fr/100 + ba×m_star), b_pred_adj는 bidC 무손실 역산(왕복 diff≤1원 검증). predict-architect 재검토(격리 3건 코드·DB 확인) → /evaluate 전 게이트 PASS → deploy-gate PASS(opt_adj 4중 불변). **G-hit floor-pass(군시설 matched): 59.74%→95.20%(+35%p)** [authoritative, 2026-05-24 정정 — `bid_records.floor_price` 조인 기준]. ⚠️ 배포일 1차 시뮬 88.74%는 floor를 `actual_expected_price×pred_floor_rate/100`로 재구성(A값·구era율 무시, 군시설 A값 53.3%)해 ~6.5%p 과소평가한 값; Codex consult 지적→실측 정정. 2b(bid_rate 인터페이스 전환)/2c(insufficient_sample UI 배지)는 미착수.
- **Phase 3 — 백테스트 alpha sweep**: 0.10~0.15 시작 → sweet spot → 0.25 방향. **(대기: 표본 ~300까지 ~8개월 또는 라이브 floor-pass 실측 7~14일 누적 후 — current n=86 insufficient_sample이라 통계 판정 불가)**
- **Phase 4 — 게이트**: `/evaluate`(G-단위/G-A안/G-hit) → deploy-gate → 7~14일 누적 효과 판정. **(Phase 2a에서 /evaluate+deploy-gate 1차 통과. 라이브 floor-pass 실측은 신규 군시설 매칭 누적 후.)**

---
_처리자: Claude Opus 4.7 / 일자: 2026-05-23 / predict-architect 검토 완료(Generator·회귀 中) / 다음 단계: writing-plans (Phase 0부터)_
_갱신: 2026-05-24 — Phase 0/1/2a 완료·배포(push d2dd667). G-hit 군시설 floor-pass 59.74%→95.20%(authoritative, floor_price 조인; 1차 시뮬 88.74%는 A값·era율 무시 과소평가). 다음: Phase 3 alpha sweep(표본 누적 대기) + 2b/2c._
