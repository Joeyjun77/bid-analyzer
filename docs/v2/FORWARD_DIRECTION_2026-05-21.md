...
## 6. 코덱스 의견 수렴 응답 (2026-05-21 추가)

`codex:codex-rescue` 서브에이전트 메타 상의 결과. 본 §3 진행 방향 + §4 질문 6건에 대한 코덱스 응답 영구 기록.

### 6.1 진행 방향 자체 평가
> "메타 점검 전환은 문서상 적절하다. 현재 병목은 **예측식 변경보다 측정 신뢰도, canonical 정합, 운영 모니터링 회복**이다. 단, V6 retire와 V2 전체 종료조건은 문서 간 구분이 필요하다."

### 6.2 6 질문 응답 요약

| # | 코덱스 응답 |
|---|---|
| Q1 단기 우선순위 | **대체로 동의** — 단 m31/m32는 무결성 major라 S7보다 앞이 타당 |
| Q2 V6 retire 기준 | **Mode B retire 가능, V2 전체 종료 불가** — §9 종료조건은 Mode A 15%도 포함. 마스터플랜 §0 자체에는 retire 조건 명시 안 됨 (문서 간 정합 필요) |
| Q3 비한전 격차 본질 | **(c) V6 모델 영역별 학습 편향 우선 검증** — 지자체 gap ≤0.0518인데 적중률 7.87%로 점추정 정확도 결정적이지 않음. (단 "데이터 편중"은 추론) |
| Q4 V6 retire 후 점추정 모듈 | **(a)/(b)만 문서 부합** — `predict_dist` 신설 또는 `agency_rate_distribution` 직접 사용. **V8 승격(c)은 근거 부족** (MEASUREMENT §6.1은 별도 함수 또는 분포 직접 사용 허용, V8 명시 없음) |
| Q5 라운드 16 트리거 | **2026-05-25 첫 canonical weekly gate 결과 수신 후 최적** — V6 retire 카운터 0주→1주 실측 전환 + S1~S3 정합 이슈와 함께 평가 가능 |
| Q6 누락 영역 | **4건 명시** (§6.4 참조) |

### 6.3 단기 우선순위 정렬 권고 (코덱스 수정)

| 순위 | 작업 | 사유 |
|---|---|---|
| 1 | **S1** weekly cron/backfill | 측정 연속성 기반 |
| 2 | **S2** canonical_ag 정규화 | 집계 신뢰도 |
| 3 | **S3** at 소스 표준화 | 정합 회복 |
| 4 | **S6** m32 UNIQUE | 중복 영구 차단 (m31 무결성 작업과 묶음) |
| 5 | **S7** Phase 17 신뢰구간 | 운영 품질 개선 |
| 6 | S4 고양시 bias | n≥10 누적 후 |
| 7 | S5 육군교육사령부 | low 유지 |

### 6.4 본 로드맵에서 누락된 영역 (코덱스 추가 발견 4건)

1. **V8 dual-run의 V6 대체 후보 평가 기준 부재** — 본 세션은 V8 fetch fix(ae18713)만 진행했으나 V6 vs V8 정확도 정량 비교 미실시. V8을 V6 대체로 승격하려면 영역별 MAE·bias 비교 필요.
2. **agency-floor V1을 UI 어느 위치에 둘지 미정** — V6 retire 후 점추정 보조 역할로 격상 가능성 (Q4 응답 (b))이지만 UI 위치·확장 방향 명세 없음.
3. **Phase 17 confidence 41.67% 낮은 원인 분석 부재** — accuracy 진단에서 발견된 신뢰구간 미흡, 본 §3.1 S7에서 "재조정" 권고만, 본질 원인 진단 미진행.
4. **Mode A 군시설 15% 목표 미달 — 목표 재정의 가능성 검토 필요** — 현 10.00%~13.33% (current-only) 측정값으로 15% 도달 가능성이 본질적으로 불투명. 목표 자체 재정의가 필요할 수 있음.

### 6.5 다음 1~2주 권고 actions (commit 단위)

1. **m33**: weekly_quality_report 누락 원인 수정 + 5/11·5/18 backfill
2. **m34**: canonical_ag NULL 정규화 보강 + at 소스 표준화 (S2 + S3 묶음)
3. **m32**: partial UNIQUE 인덱스 적용 + upsert 경로 검증

### 6.6 본 의견 수렴 결과 정리

- **즉시 진입 가능 (다음 세션 시작 시)**: m33 → m34 → m32 순서 (코덱스 권고 3 actions)
- **라운드 16 트리거**: 2026-05-25(월) 첫 canonical weekly gate 결과 수신 후
- **V6 retire 정의 명확화 필요**: 마스터플랜 §0에 "Mode B retire vs V2 전체 종료" 구분 명문화 (S2~S3 작업과 함께 정책 정정)
- **Q3 (c) V6 모델 영역별 학습 편향 가설** 검증은 비한전 영역 입찰 데이터의 V6 예측 분포 분석 필요 (별도 진단 작업)

본 코덱스 응답을 다음 세션 시작 시 §6.5 권고 3 actions로 우선 진입 결정.

_§6 추가일: 2026-05-21 / 의견 수렴자: codex:codex-rescue 서브에이전트 / 작성자: Claude Opus 4.7_
