...
**1단계 v2 (종형 보정 — 2026-04-28 hotfix)**:
```
distFitScore = floorSafe × 4 × Φ(z) × (1 − Φ(z))   where z = (자사위치 − 분포평균) / effStd
```

**용어 정정 (2026-04-28)**: 이 점수는 "1위확률"이 아니라 **"분포적합도"** — 자사 위치가 1위 사정률 분포의 중심(mean)에 얼마나 가까운지. UI 라벨 "분포적합도"로 표시 (직전엔 "1위확률"로 오해 소지). 진짜 1위 확률(N명 중 max 확률)은 2단계 Monte Carlo 시뮬레이션에서 산출.
...
해석: 1위 사정률 분포의 평균이 곧 자사가 1위가 될 가장 빈도 높은 위치. 자사가 mean에서 멀어질수록(양 극단 모두) 1위 확률 감소. 적격성 미달이면 자동 0.

**초기 단순 모델(폐기)**: `Φ(z)` 단조 증가 정의 사용 시 grid search가 항상 `mean+1.5%`로 수렴 → 비현실적 공격 추천. 운영 검증으로 즉시 종형으로 보정.
