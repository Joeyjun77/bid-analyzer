    ...
    <div style={{padding:"16px 24px 40px"}}>
      ...
      {tab==="data"&&<div style={{background:C.bg2,border:"1px solid "+C.bdr,borderRadius:10,padding:18}}>
        ...
        {recs.length===0?<div style={{textAlign:"center",padding:40,color:C.txd}}>데이터를 업로드하세요</div>:<div>
          <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}><thead><tr style={{borderBottom:"1px solid "+C.bdr}}><th style={{padding:"5px 6px",width:28}}><input type="checkbox" checked={allSel} onChange={()=>{const next={...sel};if(allSel)pagedRecs.forEach(r=>{delete next[r.id]});else pagedRecs.forEach(r=>{next[r.id]=true});setSel(next)}}/></th>{["#","기준","공고명","발주기관","유형","기초금액","예가사정율","1순위사정율","업체수","개찰일"].map(h=><th key={h} style={{padding:"5px 6px",textAlign:"left",color:C.txd,fontSize:9,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{pagedRecs.map((r,idx)=><tr key={r.id} style={{borderBottom:"1px solid "+C.bg,background:sel[r.id]?C.gold+"08":"transparent"}}><td style={{padding:"5px 6px"}}><input type="checkbox" checked={!!sel[r.id]} onChange={()=>{const next={...sel};if(next[r.id])delete next[r.id];else next[r.id]=true;setSel(next)}}/></td><td style={{padding:"5px 6px",color:C.txd}}>{dataPage*PAGE+idx+1}</td><td style={{padding:"5px 6px"}}><span style={{fontSize:8,padding:"1px 4px",borderRadius:2,background:r.era==="new"?"#5ca2":"#e552",color:r.era==="new"?"#5ca":"#e55"}}>{r.era==="new"?"신":"구"}</span></td><td style={{padding:"5px 6px",maxWidth:150,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={r.pn}>{r.pn}</td><td style={{padding:"5px 6px",maxWidth:100,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={r.ag}>{r.ag}</td><td style={{padding:"5px 6px"}}><span style={{fontSize:8,padding:"1px 5px",borderRadius:3,background:C.gold+"15",color:C.gold}}>{r.at}</span></td><td style={{padding:"5px 6px",fontFamily:"monospace",textAlign:"right"}}>{fm(r.ba)}</td><td style={{padding:"5px 6px",fontFamily:"monospace",textAlign:"right"}}>{fp(r.ar1)}</td><td style={{padding:"5px 6px",fontFamily:"monospace",textAlign:"right",fontWeight:600,color:C.gold}}>{fp(r.br1)}</td><td style={{padding:"5px 6px",textAlign:"right"}}>{r.pc.toLocaleString()}</td><td style={{padding:"5px 6px",color:C.txd}}>{r.od}</td></tr>)}</tbody></table></div>
